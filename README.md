...
- 因Cloud Alibaba版本要求，Spring Boot、Spring Cloud降级，
#### Spring Cloud

感觉着只是这样处理的话，不是很麻烦，本月试一下
- 可能会先将统一登录模块集成到Gateway中
- Gateway基于WebFlux，较MVC，在Security配置方面存在差异
- 大致理了一下，Security相关可能需要一定的工作量。模块及分层要重新设计一下。
...
- 网关why-gateway
  - 认证、鉴权
  - 日志（生产MQ消息）
  - 缓存权限相关信息，当权限变动时相关服务会发送MQ消息（发送逻辑可考虑AOP切一下），网关侧消费
