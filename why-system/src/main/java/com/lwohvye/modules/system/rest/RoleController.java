...
public class RoleController {
    ...
    private int getLevels(Integer level) {
        ...
        if (level != null && level < min) throw new BadRequestException("权限不足，你的角色级别：" + min + "，低于操作的角色级别：" + level);
        return min;
    }

    @GetMapping("/uid/{userId}")
    public ResponseEntity<Object> queryByUid(@PathVariable Long userId) {
        return new ResponseEntity<>(ResultInfo.success(roleService.findByUsersId(userId)), HttpStatus.OK);
    }
}
